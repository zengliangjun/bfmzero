...
.. code-block:: bash
    ...
    # Usage
    python examples/bfmzero/phuma_to_npz.py --input_file "/workspace.data1/ISAACSIM45/MATA/data/SPLITDATA/PHUMA/data/g1/custom/walk_chunk_0050.npy" \
...
parser.add_argument("--replace_dir", type=str, required=False, help="The path to the input motion csv file.")
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        data = np.load(self.motion_file, allow_pickle = True).item()
        fps = data['fps']
        ...
        if self.frame_range is None:
            ...
            dof_pos = data['dof_pos'][self.frame_range[0] - 1, self.frame_range[1] - self.frame_range[0] + 1]
            root_trans = data['root_trans'][self.frame_range[0] - 1, self.frame_range[1] - self.frame_range[0] + 1]
            root_ori = data['root_ori'][self.frame_range[0] - 1, self.frame_range[1] - self.frame_range[0] + 1]

        self.motion_base_poss_input = torch.from_numpy(root_trans).to(torch.float32).to(self.device)
        self.motion_base_rots_input = torch.from_numpy(root_ori).to(torch.float32).to(self.device)
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        if reset_flag and not file_saved:
            ...
            output_name = args_cli.input_file.replace(args_cli.replace_dir, "")
            path = osp.join(args_cli.out_dir, output_name)
